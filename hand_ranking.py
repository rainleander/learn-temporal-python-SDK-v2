...
def rank_hand(hand: List[Card]) -> Tuple[int, List[int]]:
    ...
    if is_straight and is_flush:
        return (
            9,
            ranks if ranks[-1] != 2 else [5, 4, 3, 2, 1],
        )  # Straight flush (ace-low straight flush)
    if 4 in rank_counts.values():
        return (
            8,
            [r for r in rank_counts if rank_counts[r] == 4]
            + [r for r in rank_counts if rank_counts[r] != 4],
        )  # Four of a kind
    if 3 in rank_counts.values() and 2 in rank_counts.values():
        return (
            7,
            [r for r in rank_counts if rank_counts[r] == 3]
            + [r for r in rank_counts if rank_counts[r] != 3],
        )  # Full house
    if is_flush:
    ...
    if 3 in rank_counts.values():
        return (
            4,
            [r for r in rank_counts if rank_counts[r] == 3]
            + sorted([r for r in rank_counts if rank_counts[r] != 3], reverse=True),
        )  # Three of a kind
    if 2 in rank_counts.values():
        ...
        if len(pairs) == 2:
            return (
                ...
                + [r for r in rank_counts if rank_counts[r] == 1],
            )  # Two pairs
        else:
            return (
                2,
                pairs
                + sorted([r for r in rank_counts if rank_counts[r] == 1], reverse=True),
            )  # One pair
    return 1, ranks  # High card


async def draw_cards(
    game_state: GameState, player_idx: int, discard_indices: List[int]
) -> None:
